...
const CAP: Record<OPTType, number> = { OPT: 90, 'STEM OPT': 150 };

type OPTType = 'OPT' | 'STEM OPT';
...
function pd(s: string) { return noon(parseISO(s)); }
function todayNoon()   { return noon(new Date()); }
function span(a: Date, b: Date) { return Math.max(0, differenceInDays(b, a)); }

function calcStats(data: OPTTrackerData) {
  const t        = todayNoon();
  const optStart = pd(data.setup.start_date);
  const optEnd   = pd(data.setup.end_date);
  const winEnd   = isBefore(t, optEnd) ? t : optEnd;
  const cap      = CAP[data.setup.opt_type];
  const isExpired   = isBefore(optEnd, t);
  const notStarted  = !isAfter(t, optStart);
  ...
  const intervals = data.periods
    .map(p => {
      ...
      const e = p.end_date ? pd(p.end_date) : t;
      return {
